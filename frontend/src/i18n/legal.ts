    ...
    privacy: {
      ...
      sections: [
        ...
        { title: '보유 기간과 삭제', paragraphs: [
          ...
          '프로젝트 삭제와 계정 탈퇴는 서로 다른 기능입니다. 보관함에서 완료한 현지화 프로젝트를 삭제하면 해당 프로젝트와 연결된 파일을 삭제하며, 다른 프로젝트나 계정은 유지됩니다. 공개 GitHub 피드백은 계정 탈퇴만으로 자동 삭제되지 않습니다. 삭제가 필요하면 이슈 주소와 함께 아래 연락처로 요청해 주세요.'
        ] },
        ...
        { title: '외부 서비스와 데이터 전달', paragraphs: [
          ...
          'OGQ 콘텐츠 검색에는 검색어로 사용되는 프로젝트 문구가 OGQ API에 전달됩니다. 관련 콘텐츠를 찾기 위한 기능이므로 검색 문구에 개인정보를 포함하지 마세요. 피드백 제출 시 GitHub에 메시지와 계정 이름·이메일 또는 계정 식별자를 포함한 공개 이슈가 생성될 수 있습니다. 비밀번호나 개인 연락처 등 공개를 원하지 않는 정보는 입력하지 마세요.'
        ] },
  ...
  ja: {
    privacy: {
      ...
      sections: [
        ...
        { title: '外部サービスとデータの送信', paragraphs: ['Supabaseをデータベース・ファイル保存・Google認証に、Vercel・Renderをサイトとサーバーのホスティングに使用します。Google・Naverは選択したソーシャルログインに使用します。', '有効な機能とサーバー設定に応じて、OpenAI・Groq・Google Geminiに画像や必要な画像領域、認識文、翻訳先言語、生成指示、チェック対象のプロジェクト情報を送信する場合があります。目的はOCR、翻訳、画像生成、文言・フォント分析、公開前チェックです。', '処理場所や保存方針は提供者と運用設定により異なり、国外で処理される場合があります。関連情報は下記へお問い合わせください。Glocalizerはアップロード画像を自社AIモデルの学習に使用しません。外部提供者の処理には各社の方針が適用されます。', 'OGQコンテンツ検索では、関連コンテンツを探すために検索語として使用するプロジェクトの文言をOGQ APIに送信します。検索語に個人情報を含めないでください。フィードバックを送信すると、メッセージとアカウントの名前・メールアドレスまたは識別子を含む公開GitHubイシューが作成される場合があります。パスワードや個人の連絡先など、公開したくない情報は記入しないでください。'] },
  ...
  zh: {
    privacy: {
      ...
      sections: [
        ...
        { title: '保存期限与删除', paragraphs: ['会员信息和会员项目保存至账号注销或相应数据删除。设有期限的访客项目按照显示的到期时间进行清理。服务器默认期限为创建后 24 小时，可能随运营设置变化。', '邮箱和 Naver 登录令牌自签发起最长有效 30 天。退出登录会清除此浏览器保存的登录信息。Google 登录还适用认证提供方的会话政策。', '服务日志仅在安全和故障处理所需的最短期间保存。依法必须保留的信息会在规定期限内单独保存。注销时依次删除项目、图片、个人资料及关联认证信息，删除后无法恢复。', '删除项目与注销账号是不同操作。在归档中删除已完成的本地化项目时，会删除该项目及关联文件，其他项目与账号保持不变。公开的 GitHub 反馈不会随账号注销自动删除。如需删除，请将相关 issue 链接发送至下方邮箱。'] },
        { title: '浏览器存储', paragraphs: ['登录令牌、账号信息及网站语言保存在本地存储中，工作状态保存在会话存储中。您可以在浏览器设置中清除网站数据，但可能因此退出登录或无法恢复工作。使用公共设备后请退出登录。'] },
        { title: '外部服务与数据传输', paragraphs: ['Supabase 用于数据库、文件存储和 Google 认证；Vercel 和 Render 用于网站和服务器托管。Google 和 Naver 提供您选择的社交登录方式。', '根据启用的功能和服务器设置，可能向 OpenAI、Groq 或 Google Gemini 传输图片或所需图片区域、识别文字、目标语言、生成指令及待检查项目内容，用于 OCR、翻译、图片生成、文字与字体分析及发布前检查。', '处理地点和保存政策因提供方及运营设置而异，数据可能在境外处理。有关处理信息可通过下方联系方式咨询。Glocalizer 不会将上传图片用于训练自有 AI 模型；外部提供方按其各自政策处理数据。', 'OGQ内容搜索会将用作关键词的项目文案发送至OGQ API，以查找相关内容。请勿在这些关键词中包含个人信息。提交反馈可能创建公开的 GitHub issue，其中包含消息及账号姓名、邮箱或账号标识。请勿填写密码、私人联系方式或其他不希望公开的信息。'] },
