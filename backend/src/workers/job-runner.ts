...
import { claimNextQueuedJob, markJobCompleted, markJobFailed, markJobFailedOrRequeue } from '../repositories/job.repository.js';
...
export async function processClaimedJob(job: import('../types/job.js').JobRow, allowRequeue: boolean): Promise<void> {
  ...
  try {
    ...
    if (allowRequeue) {
      ...
      await markJobFailed(job.id, errorCode, errorMessage);
...
export async function processNextJob(): Promise<boolean> {
  ...
  if (!job) return false;
  await processClaimedJob(job, true);
