...
async function validateAndStoreAsset(asset: AssetRow): Promise<AssetUploadResult> {
  if (!asset.original_path) {
    return { assetId: asset.id, status: 'failed', errorCode: 'UPLOAD_NOT_COMPLETED', errorMessage: '업로드 경로가 없습니다.' };
  }
  ...
  if (!buffer) {
    await updateAsset(asset.id, {
      ...
      errorMessage: '스토리지에서 파일을 찾을 수 없습니다. 업로드가 완료되었는지 확인해주세요.',
    });
    return {
      assetId: asset.id,
      status: 'failed',
  ...
  try {
    ...
    await updateAsset(asset.id, {
      ...
      errorMessage: '이미지를 디코딩할 수 없습니다.',
    });
    return { assetId: asset.id, status: 'failed', errorCode: 'IMAGE_DECODE_FAILED', errorMessage: '이미지를 디코딩할 수 없습니다.' };
  }
  ...
  if (!metadata.format || metadata.format !== expectedFormat) {
    await updateAsset(asset.id, {
      ...
      errorMessage: `선언된 형식(${asset.mime_type})과 실제 파일 형식이 일치하지 않습니다.`,
    });
    return {
      assetId: asset.id,
      status: 'failed',
  ...
  if (!metadata.width || !metadata.height) {
    await updateAsset(asset.id, {
      status: 'failed',
      errorCode: 'IMAGE_DECODE_FAILED',
      errorMessage: '이미지 크기를 확인할 수 없습니다.',
    });
    return { assetId: asset.id, status: 'failed', errorCode: 'IMAGE_DECODE_FAILED', errorMessage: '이미지 크기를 확인할 수 없습니다.' };
  }
  ...
  if (metadata.width > env.MAX_IMAGE_WIDTH || metadata.height > env.MAX_IMAGE_HEIGHT) {
    await updateAsset(asset.id, {
      ...
      errorMessage: `이미지 해상도는 ${env.MAX_IMAGE_WIDTH}x${env.MAX_IMAGE_HEIGHT}를 초과할 수 없습니다.`,
    });
    return {
      assetId: asset.id,
      status: 'failed',
